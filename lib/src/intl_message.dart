...
abstract class Message {
  ...
  /// the actual examples.
  Map<String, dynamic>? get examples => parent == null ? const {} : parent!.examples;
  ...
  String? checkValidity(
      MethodInvocation node, List arguments, String? outerName, List<FormalParameter> outerArgs,
    ...
    if (!checkArgs(args, parameterNames as List<String?>)) {
    ...
    var messageNameArgument = arguments.firstWhere(
        (eachArg) => eachArg is NamedExpression && eachArg.name.label.name == 'name',
        orElse: () => null);
    ...
    var simpleArguments = arguments.where(
        (each) => each is NamedExpression && ['desc', 'name'].contains(each.name.label.name));
    ...
    if (hasParameters) {
      var exampleArg =
          arguments.where((each) => each is NamedExpression && each.name.label.name == 'examples');
  ...
  static String? classPlusMethodName(MethodInvocation node, String? outerName) {
    ...
    return classDeclaration == null ? null : '${classDeclaration.name.token}_$outerName';
  }
...
class CompositeMessage extends Message {
  ...
  CompositeMessage(List<Message> this.pieces, [ComplexMessage? parent]) : super(parent) {
    for (var x in pieces!) {
      x.parent = this;
    }
  }
...
class VariableSubstitution extends Message {
  ...
  String? get variableName {
    return arguments != null && index != null ? arguments![index!] : _variableName;
  }
...
class MainMessage extends ComplexMessage {
  ...
  String? checkValidity(
    ...
    return super.checkValidity(node, arguments, outerName, outerArgs,
        nameAndArgsGenerated: nameAndArgsGenerated, examplesRequired: examplesRequired);
  }
  ...
  @override
  String toCode() => throw UnsupportedError('MainMessage.toCode requires a locale');

  @override
  String toJson() => throw UnsupportedError('MainMessage.toJson requires a locale');
  ...
  String toOriginalCode({bool includeDesc = true, bool includeExamples = true}) {
...
abstract class SubMessage extends ComplexMessage {
  ...
  Map<String, dynamic> argumentsOfInterestFor(MethodInvocation node) {
    ...
    return {
      for (var node in others) node.name.label.token.value() as String: node.expression,
    };
  }
  ...
  String expanded([Function? transform = _nullTransform]) {
    ...
    var clauses =
        attributeNames.where((key) => this[key] != null).map(fullMessageForClause).toList();
  ...
  String toCode() {
    ...
    args.fold<StringBuffer>(
        out, (buffer, arg) => buffer..write(", $arg: '${(this[arg] as Message).toCode()}'"));
    out.write(')}');
    return out.toString();
  }
...
class Select extends SubMessage {
  ...
  void operator []=(String attributeName, dynamic rawValue) {
    ...
    if (validSelectKey.stringMatch(attributeName) == attributeName) {
      ...
      throw IntlMessageExtractionException("Invalid select keyword: '$attributeName', must "
          "match '$selectPattern'");
    }
  }
